module.exports = {
  ...
  langs: {
    en: {
      ...
      returnListPending: "📋 | Pending Groups (%1)\n━━━━━━━━━━━━━━\n\n%2\n\n📌 Reply with:\n✅ '{pn} approve <numbers>' to approve\n❌ '{pn} cancel <numbers>' to reject\n🧪 Example: 'pen approve 1 2 3'",
  ...
  onStart: async function({ api, event, getLang, commandName, args }) {
    ...
    try {
      ...
      setTimeout(() => {
        if (replyMsg?.messageID && global.GoatBot.onReply.has(replyMsg.messageID)) {
  ...
  onReply: async function({ api, event, Reply, getLang, commandName }) {
    ...
    if (invalidNumbers.length > 0) {
      return api.sendMessage(getLang("invalidNumber", invalidNumbers.join(', ')), threadID, messageID);
    }
