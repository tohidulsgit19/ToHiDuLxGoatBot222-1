...
function getRole(threadData, senderID) {
	...
	// Check VIP access for role 3
	if (hasVipAccess(senderID)) {
		return Math.max(3, adminBox.includes(senderID) ? 1 : 0);
	}
	
	return adminBot.includes(senderID) ? 2 : adminBox.includes(senderID) ? 1 : 0;
}

function hasVipAccess(userID) {
			...
			if (needRole > role) {
				if (!hideNotiMessage.needRoleToUseCmd) {
					if (needRole == 1)
						...
						return await message.reply("❌ YOU ARE NOT VIP BBY\n💎 Use '.vip rules' to see VIP requirements");
				}
				else {
					return true;
				}
			...
			if (needRole > role) {
				if (!hideNotiMessage.needRoleToUseCmdOnReaction) {
					if (needRole == 1)
						...
						return await message.reply("❌ ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀ ᴠɪᴘ ʙʙʏ~ 😿 \n ᴜꜱᴇ .vip rules ᴛᴏ ꜱᴇᴇ ᴠɪᴘ ʀᴇǫᴜɪʀᴇᴍᴇɴᴛꜱ ✨");
				}
				else {
					return true;
				}
